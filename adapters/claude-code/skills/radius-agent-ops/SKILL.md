...
# Signing key
export RADIUS_ACCOUNT=radius-dev  # Foundry keystore account name
```

Import the key once, then use the account name in examples:

```bash
cast wallet import "$RADIUS_ACCOUNT" --interactive
```

- Prefer Foundry keystores (`cast wallet import`) and `--account`; avoid passing private keys on command lines.
...
|-----------|----------------|--------------------|-----------------|
| Wallet address | `cast wallet address --account "$RADIUS_ACCOUNT"` | `Account.from_key(...)` | `privateKeyToAccount(...)` |
...
```bash
ADDR=$(cast wallet address --account "$RADIUS_ACCOUNT")
...
```bash
BYTECODE=0x608060...
DEPLOY_TX=$(cast send --create "$BYTECODE" --rpc-url "$RADIUS_RPC_URL" --account "$RADIUS_ACCOUNT")

# Read function
cast call 0xContractAddress "getCount()(uint256)" --rpc-url "$RADIUS_RPC_URL"

# Write function
WRITE_TX=$(cast send 0xContractAddress "increment()" --rpc-url "$RADIUS_RPC_URL" --account "$RADIUS_ACCOUNT")
